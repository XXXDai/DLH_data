...
INPUT_DATASET_ID = "D10013"  # 输入数据集标识，字符串
OUTPUT_DATASET_ID = "D10015"  # 输出数据集标识，字符串
...
def resolve_symbols(exchange: str, base_dir: Path) -> list:
    """解析单个交易所的交易对列表。"""
    return sorted(set(cex_config.get_future_symbols(exchange)) | set(list_input_symbols(base_dir)))
...
def iter_available_dates(exchange: str, base_dir: Path, symbol: str) -> list:
    ...
    if not symbol_dir.exists():
        return []
    dates = set()
    pattern = "*.zip" if exchange == "bitget" else f"{symbol}*.csv.gz"
