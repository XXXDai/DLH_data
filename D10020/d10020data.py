...
def build_file_path(hour_tag: str) -> Path:
    return DATA_DIR / hour_tag / f"polymarket_orderbook_ss_1m_{hour_tag}.parquet"
...
def market_analysis(df: pd.DataFrame) -> None:
    if df.empty:
        print("数据为空")
        return
...
def main() -> None:
    ...
    if not df_range.empty:
        market_analysis(df_range)
    start_ts = int(datetime(2026, 1, 8, 15, 0, tzinfo=timezone.utc).timestamp() * 1000)
    end_ts = int(datetime(2026, 1, 8, 15, 30, tzinfo=timezone.utc).timestamp() * 1000)
