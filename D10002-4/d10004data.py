...
DATA_DIR = Path("data/dylan/src/bybit_future_orderbook_rt_ss_1s")  # 数据目录，路径
...
def build_file_path(base_dir: Path, symbol: str, hour_str: str) -> Path:
    file_name = f"{symbol}-bybit_future_orderbook_rt_ss_1s-{hour_str}.json"
...
def main() -> None:
    ...
    df["datetime"] = pd.to_datetime(df["ts"], unit="ms", utc=True)
    print(f"记录数: {len(df)} (预期约3600条/小时)")
