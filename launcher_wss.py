...
WSS_CONFIG = load_wss_config()  # WSS配置字典，映射
S3_BUCKET_NAME = str(WSS_CONFIG["s3"]["bucket_name"]).strip()  # S3桶名称，字符串
S3_PREFIX = normalize_s3_prefix(str(WSS_CONFIG["s3"]["prefix"]))  # S3目录前缀，路径
...
WS_WRITE_BUFFER_LINES = int(WSS_CONFIG["write"]["buffer_lines"])  # WS单文件写入缓冲行数阈值，行
WS_WRITE_BUFFER_BYTES = int(WSS_CONFIG["write"]["buffer_bytes"])  # WS单文件写入缓冲字节阈值，字节
WS_WRITE_BUFFER_INTERVAL_SECONDS = float(WSS_CONFIG["write"]["buffer_interval_seconds"])  # WS单文件写入缓冲刷新间隔，秒
FILE_SEGMENT_SECONDS = int(WSS_CONFIG["write"]["file_segment_seconds"])  # WS文件切分间隔，秒
FILE_SEGMENT_FORMAT = build_time_partition_format(FILE_SEGMENT_SECONDS)  # WS文件分区格式，字符串
...
EXCHANGE_ENABLED = dict(WSS_CONFIG["exchange_enabled"])  # 交易所总开关映射，映射
SPOT_SYMBOLS = dict(WSS_CONFIG["spot_symbols"])  # 现货交易对映射，映射
FUTURE_PERPETUAL_SYMBOLS = dict(WSS_CONFIG["future_perpetual_symbols"])  # 永续交易对映射，映射
FUTURE_DELIVERY_FAMILIES = dict(WSS_CONFIG["future_delivery_families"])  # 交割合约家族映射，映射
DATASET_SUPPORT = dict(WSS_CONFIG["dataset_support"])  # 实时数据集支持矩阵，映射
