            ...
            for parts in reader:
                ...
                if normalized:
                    ...
                    if writer is None:
                        ...
                        if tmp_path.exists():
                            tmp_path.unlink()
                        file_obj, writer = open_gzip_csv_writer(tmp_path, SPOT_TRADE_FIELDS)
...
def split_okx_zip(archive_path: Path, symbol: str, base_dir: Path) -> int:
    """拆分OKX压缩成交文件。"""
    writer_handles = {}
    csv_writers = {}
    temp_output_paths = {}
...
def download_okx_archive(base_dir: Path, symbol: str, url: str, failure_key: str, fail_path: Path, synced_days: int = 0, stage_text: str = "") -> bool:
    ...
    count = split_okx_zip(archive_path, symbol, base_dir)
    archive_path.unlink()
...
def download_bitget_day(base_dir: Path, symbol: str, date_str: str, fail_path: Path, synced_days: int = 0) -> bool:
    ...
    if tmp_path.exists():
        tmp_path.unlink()
    tmp_file_obj, tmp_writer = open_gzip_csv_append_writer(tmp_path, SPOT_TRADE_FIELDS)
    shard_count = 0
    row_count = 0
